...
class Nextage(PyBulletRobot):
    ...
    def __init__(
        ...
        urdf_path = os.path.join(parent_dir, 'assets', 'nextage_description', 'urdf', 'NextageAOpen.urdf')

# Now you can use urdf_path in your code
        ...
        super().__init__(
            ...
            joint_indices=np.array([5, 6, 7, 8, 9, 10]), # Left arm indices
            joint_forces=np.array([120.0, 120.0, 120.0, 120.0, 170.0, 170.0]), # TODO Adjust
        )

        self.fingers_indices = np.array([10])
    ...
    def set_action(self, action: np.ndarray) -> None:
        ...
        target_angles = np.concatenate((target_arm_angles, [target_fingers_width / 2, target_fingers_width / 2]))[:6]
