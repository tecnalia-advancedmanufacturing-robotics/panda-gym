...
from panda_gym.envs.robots.converging import Converging
...
class ConvergingReachEnv(RobotTaskEnv):
    """Reach task wih Converging robot.
    ...
    def __init__(
        ...
        render_target_position: Optional[np.ndarray] = np.array([0.5, 0.5, 0.0]),
        ...
        robot = Converging(sim, block_gripper=True, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
